 ...
 *
 */
public class ProgressAsyncWebProcessor extends DefaultAsyncWebProcessor<SimpleMessage>{
	

	public ProgressAsyncWebProcessor(PrintWriter out, String asynCallback) {
		this(out, new StringMessageHolder());
		if(StringUtils.isNotBlank(asynCallback))
			this.asynCallback = asynCallback;
	}

	public ProgressAsyncWebProcessor(PrintWriter out, AsyncMessageHolder holder) {
		this(out, holder, SpringApplication.getInstance().getBean(AsyncTaskExecutor.class));
	}
	public ProgressAsyncWebProcessor(PrintWriter out, AsyncMessageHolder holder, AsyncTaskExecutor asyncTaskExecutor) {
...
//			this.doAsynWithSingle((i+1)==taskCount, i, taskDatas, creator);
			AsyncTask task = creator.create(i, taskDatas, getAsynMessageTunnel());
