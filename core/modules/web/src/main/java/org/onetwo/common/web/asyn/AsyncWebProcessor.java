 ...
 *
 */
public class AsyncWebProcessor<T> {
	
	private static final Logger logger = MyLoggerFactory.getLogger(AsyncWebProcessor.class);
	
	private PrintWriter out;
	private AsyncMessageTunnel<T> asynMessageTunnel;
	...
	AsyncWebProcessor(PrintWriter out, AsyncMessageTunnel<T> holder, String asynCallback) {
	...
	AsyncWebProcessor(PrintWriter out, AsyncMessageTunnel<T> holder, AsyncTaskExecutor asyncTaskExecutor) {
		super();
		this.out = out;
//		this.dataCountPerTask = taskInterval;
	...
	public AsyncMessageTunnel<?> getAsynMessageTunnel() {
	...
	public void flushAndClearTunnelMessage(){
		List<T> meesages = asynMessageTunnel.getAndClearMessages(); 
