...
//		return result;
	...
	public void consumeMessage(JmsConsumeContext context) {
		try {
			context.getConsumeAction().invokeListener();
	...
	//		currentConetxt.markWillSkipConsume();
			...
			Throwable finalEx = LangUtils.getFinalCauseException(e);
			if (finalEx instanceof MessageOnlyServiceException) {
				// 此异常无需回滚
				if (logger.isInfoEnabled()) {
					String msg = buildErrorMessage(context);
					logger.info("jms consumer will not rollback: {}", msg);
...
//	public void consumeMessageBak(JmsConsumeContext context) {
...
//	}
	...
	protected String buildErrorMessage(JmsConsumeContext context) {
		...
		String msg = "jms-consumer["+context.getConsumeGroup()+"] consumed message error. " + 
					"id: " +  msgId + 
