...
<%
	...
	case button:
		%>
		<a href="${field.value }" class="btn btn-primary  ${field.cssClass }" ${field.attributesHtml}>${field.label }</a>
		<%
		break;
	case checkbox:
	...
	case submit:
		%>
		${ff.label }<input name="" type="submit" value="${field.label }" class="btn btn-primary ${field.cssClass }" <c:if test="${field.showLoadingText}">data-loading-text="正在提交……"</c:if> ${field.attributesHtml} />
		<%
		break;
	default:
