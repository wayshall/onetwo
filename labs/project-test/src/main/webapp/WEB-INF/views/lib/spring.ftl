 ...
 *    or CSS styles or size
 -->
<#macro form path action method="post" multipart=false attributes="">
    <#local atualMethod=method />
    <#if atualMethod?lower_case != "get">
    	<#local atualMethod="post" />
    </#if>
    <#assign nestedPath=path>
    <form name="${path}Form" action="${action}" method="${atualMethod}" <#if multipart>ENCTYPE="multipart/form-data"</#if> ${attributes}>
...
<#macro formInput path attributes="" fieldType="text">
    ...
    <#list status.errorMessages as error> <b>${error}</b> <br> </#list>
</#macro>
