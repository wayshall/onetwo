...
<ul>
	<%
	if(page.isHasPre()){ %>
		<li><a href="<%= pageLink(action, 1)%>" style="background:none">首页</a></li>
		<li><a href="<%= pageLink(action, page.getPrePage())%>" style=" background:none">上一页</a></li>
	<%} 
	String pageClass = "";
	...
	%>
		<li><a href="<%=pageLink(action, pageNumber) %>" <%=pageClass%>><%=pageNumber %></a></li>
	<%
	}//end for
	
	if(page.isHasNext()){%>
		<li><a href="<%=pageLink(action, page.getNextPage()) %>" style=" background:none">下一页</a></li>
		<li><a href="<%=pageLink(action, page.getTotalPages()) %>" style=" background:none">尾页</a></li> 
	<%} //end if%>
</ul>
 </div>
 <div>
